...
class Cart < ApplicationRecord
  ...
  def group_items(array)
    ...
    array.shift
    array, output, shift_counter = first_iteration(array, output, shift_counter)
    shift_counter.times { array.shift }
    array.each do |array_e|
      output.each_with_index do |output_e, output_i|
        if output_e[0] === array_e[0]
          if output_i == output.size - 1
        ...
        if output_e[0].size == 3 && output[output_i - 1][0].size == 4
        ...
        elsif output_e[0].size < 4
          output_e[0] << array_e[0]
          output_e[1] += array_e[2]
          break
        elsif output_e[0].size == 4 && output.none? { |e| e[0].size < 4 }
          output_e[0] << array_e[0]
          output_e[1] += array_e[2]
          break
        end
  ...
  def first_iteration(array, output, shift_counter)
    shift_counter = 0
    array.each do |array_e|
      ...
      output.each do |output_e|
        ...
        next if output_e[0] === array_e[0]

        if output_e[0].size < 5
          output_e[0] << array_e[0]
          output_e[1] += array_e[2]
          array_e[1] -= 1
        end
